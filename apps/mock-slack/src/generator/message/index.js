...
class TimestampGenerator {
  ...
  addRandomTime(ts) {
    return toSlackTs(
      toMillis(ts) + randomNumber(this.minTimeDiff, this.maxTimeDiff),
    );
  }
  ...
  next() {
    let nextTs = this._isThread
      ? this.addRandomTime(this.ts)
      : this.subtractRandomTime(this.ts);
  ...
  thread(threadTimestamp) {
    const threadTimestampGenerator = new TimestampGenerator(
      this.maxTimeDiff,
      this.minTimeDiff,
      threadTimestamp,
    );
...
class MessageGenerator {
  ...
  generateThread(timestampGenerator) {
    ...
    for (
      let i = 0, ts = nextTs();
      i < parent.reply_count && ts <= toSlackTs(Date.now());
      i++, ts = nextTs()
    ) {
