...
app.use((req, res, next) => {
  console.log(req.url, req.params, req.body, req.headers);
  next();
});
...
app.get('/oauth/authorize', (req, resp) => {
  ...
  resp.redirect(
    `${redirect}?code=${authCode}&state=${encodeURIComponent(state)}`,
  );
});

app.post('/api/oauth.access', (req, resp) => {
  if (req.body.code.startsWith('mock-slack.error.return:')) {
    const simulatedErrorCode = req.body.code.split(
      'mock-slack.error.return:',
    )[1];

    return resp.status(200).send({
      ok: false,
      error: simulatedErrorCode,
    });
  }

  const auth = req.headers['authorization'] || '';
  const token =
    Buffer.from(auth.match(/Basic (.*)/)[1], 'base64').toString() || '';
...
app.get('/api/conversations.list', (req, resp) => {
  const { items, nextCursor } = getItems(
    'conversations',
    req.query.cursor,
    conversations,
    CONVERSATIONS_BATCH,
